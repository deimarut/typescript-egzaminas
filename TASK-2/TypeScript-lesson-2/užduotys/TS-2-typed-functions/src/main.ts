...
type functionMultiply = (a: number, b: number) => number;

  const multiply: functionMultiply = (a, b) => {
    return a * b;
  };

type functionPower = (base: number, power: number) => number;

  const power: functionPower = function (base, power) {
    return base ** power;
  };

type functionSquareRoot = (number: number) => number;
  
const squareRoot: functionSquareRoot = (number) => number ** (1 / 2);

type functionRoot = (base: number, nthRoot: number) => number;

const root: functionRoot = function (base, nthRoot) {
    return base ** (1 / nthRoot);
  }

  type functionPrintBlueText = (text: string) => void;
  
  const printBlueText: functionPrintBlueText = (text) => console.log(`%c${text}`, 'color: #0000ee');
